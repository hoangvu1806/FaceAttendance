...
class FaceAccessApp:
    ...
    def setup_ui(self):
        ...
        # config_frame = ttk.LabelFrame(ctrl, text="Configuration", padding=5)
        # config_frame.pack(fill=tk.X, pady=10)
        
        # ttk.Label(config_frame, text=f"T_reject: {config.T_REJECT}", 
        #          font=("Arial", 9)).pack(anchor=tk.W)
        # ttk.Label(config_frame, text=f"T_accept: {config.T_ACCEPT}", 
        #          font=("Arial", 9)).pack(anchor=tk.W)
        # ttk.Label(config_frame, text=f"Voting: {config.VOTING_THRESHOLD}/{config.VOTING_FRAMES}", 
        #          font=("Arial", 9)).pack(anchor=tk.W)
        # ttk.Label(config_frame, text=f"Cooldown: {config.ATTENDANCE_COOLDOWN}s", 
        #          font=("Arial", 9)).pack(anchor=tk.W)
        # ttk.Label(config_frame, text="Edit config.py to change", 
    ...
    def recognize_frame(self, face_img, is_frontal=True, track_id=None, liveness_score=1.0):
        ...
        emb = self.embedder.extract(face_img, check_quality=False)  # Skip quality check for speed
        if not emb:
            return {
                "name": None,
                "score": 0.0,
    ...
    def capture_sample(self, event=None):
        ...
        if faces:
            _, face_img = faces[0]
            emb = self.embedder.extract(face_img)
            if emb:
